  ...
  const fetchHistory = async (sid) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessages([{ text: "Hello! I'm NewsBot, your AI assistant for the latest news. How can I help you?", sender: 'bot' }]);
  ...
  const handleNewSession = async () => {
    ...
    try {
      ...
      setSessionId(newSessionId);
       setMessages([
        { text: "New session started. Ask me anything about the news!", sender: 'bot' }
       ]);
    } catch (error) {
  ...
  const Message = ({ msg }) => {
    ...
    return (
      <div className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end' : 'justify-start'}`}>
        ...
        <div className={`max-w-md p-3 rounded-xl ${isUser ? 'bg-blue-600 text-white rounded-br-none' : 'bg-slate-700 text-slate-200 rounded-bl-none'}`}>
